...
| Crate | Path | Purpose |
|-------|------|---------|
| `big-code-analysis` | `./` (root) | Library: the metric walk, output formats, suppression, VCS metrics; the published API surface |
| `big-code-analysis-ast` | `big-code-analysis-ast/` | Parse and classification layer (#1376): generated kind enums, `LANG`, `Node`, `Checker` / `Getter` / `Alterator`, the C preprocessor pass. Internal plumbing the root pins at `=X.Y.Z`; depend on the root, not on it |
...
- `src/lib.rs` — public re-exports; this is the published API surface.
  Everything the parse layer defines reaches the metric modules through
  one `pub(crate) use big_code_analysis_ast::*` here, so `use crate::*`
  in a metric file still names every token enum, tag and classifier.
- `big-code-analysis-ast/src/languages/` — one `language_<lang>.rs` per
  supported language, generated by `enums/`. These modules deliberately
  mirror each other, as do the per-language `checker/`, `getter/` and
  `src/metrics/<metric>/` siblings. A bug in one language module
  typically exists in several — fix all affected siblings together.
- `big-code-analysis-ast/src/` — `langs.rs` (`LANG`, `AnyParser`),
...
- `src/spaces.rs` / `src/spaces/` — `Ast`, `analyze`, `FuncSpace`, and
  the metric walk; `src/metric_suite.rs` — the `MetricSuite` supertrait
  that keys the 13 per-metric impls on a parser; `src/macros/mod.rs` —
...
- When a change touches metric computation, AST traversal, or anything
  under `big-code-analysis-ast/src/languages/`, exercise **every** language affected — passing
...
`big-code-analysis-ast/src/getter.rs`. It is intentionally not part of
