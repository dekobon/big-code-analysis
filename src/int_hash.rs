...
//!
//! Three of them are hot enough to matter: the node-id keyed
//! [`NestingMap`](crate::spaces::NestingMap) the walker threads through
//! `Cognitive`, the line-number sets behind `Ploc` / `Cloc` in
//! [`crate::metrics::loc`], and the `kind_id`-keyed operator map in
//! [`HalsteadMaps`](crate::metrics::halstead::HalsteadMaps). The first
//! two are probed at least once per AST node; the third is probed once
//! per node the language's `Getter` classifies as a non-primitive
//! operator, which over the corpus repositories works out at roughly
//! one probe per 15 bytes of source.
//!
...
/// source line numbers, and tree-sitter `kind_id` grammar symbols
...
mod tests {
    ...
    /// `write_usize` for the node-id and line-number collections, and
