...
/// `NodeIdHasher`'s integer writes must all agree for the same value.
///
...
#[test]
fn node_id_hasher_integer_writes_agree() {
    use std::hash::Hasher;

    use crate::spaces::NodeIdHasher;

    let hash_of = |write: &dyn Fn(&mut NodeIdHasher)| {
        let mut h = NodeIdHasher::default();
        write(&mut h);
        h.finish()
    };
    ...
    for value in [0_u64, 1, 42, 0x0123_4567, u64::from(u32::MAX), u64::MAX] {
        ...
        let via_usize = hash_of(&|h| h.write_usize(value as usize));
        assert_eq!(
            via_u64, via_usize,
            "write_u64 and write_usize must agree for {value:#x}"
        );
...
/// `rustc-hash` added this in 2.0; a first cut of `NodeIdHasher` copied
/// the 1.x algorithm and reproduced exactly the defect upstream removed.
#[test]
fn node_id_hasher_finish_rotates_entropy_down() {
    use std::hash::Hasher;
