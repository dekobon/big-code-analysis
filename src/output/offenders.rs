...
/// Short, human-readable rule descriptions used by aggregated output
/// formats (`SARIF` rule short-description, GitLab Code Climate
/// `description` prefix). Metrics not listed here fall back to the
/// metric name itself — never fail.
pub(crate) const RULE_DESCRIPTIONS: &[(&str, &str)] = &[
    (
        "cyclomatic",
...
/// Look up the long-form rule description for `metric`. Returns
/// `None` when the metric is not in `RULE_DESCRIPTIONS`; callers
/// choose their own fallback (sarif falls back to the metric id;
/// code-climate omits the long-form prefix).
