...
/// its output value.
///
/// Kept `pub` so the macro-generated `action::<T>` dispatcher and the
/// per-callback `impl Callback` blocks (in this crate and in
/// `bca-web`) can name it, but `#[doc(hidden)]` because its `call`
/// method is bound on the doc-hidden [`ParserTrait`] — a downstream
/// impl cannot be written against the documented surface alone, so it
/// shares `ParserTrait`'s holding-pattern visibility rather than
/// advertising a contract we do not yet support. See STABILITY.md.
#[doc(hidden)]
pub trait Callback {
...
/// `mk_code!` macro.
///
/// Kept `pub` so the doc-hidden `Parser<T>` plumbing and the
/// `mk_code!` expansion can name it, but `#[doc(hidden)]` because it
/// is reachable from documented API only through the doc-hidden
/// `Parser` / `ParserTrait` types; it shares their holding-pattern
/// visibility. See STABILITY.md.
#[doc(hidden)]
