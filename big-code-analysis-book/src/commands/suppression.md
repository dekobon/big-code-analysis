...
The native dialect uses the `bca:` namespace and the `allow` verb,
mirroring Rust's `#[allow(clippy::…)]` attribute. Four forms:

| Marker                              | Scope             | Effect                          |
| ----------------------------------- | ----------------- | ------------------------------- |
| `bca: allow`                        | Enclosing function | Suppress every metric           |
| `bca: allow(metric, ...)`           | Enclosing function | Suppress only the listed metrics |
| `bca: allow-file`                   | File              | Suppress every metric           |
| `bca: allow-file(metric, ...)`      | File              | Suppress only the listed metrics |
...
ignored; for file-wide silencing use the explicit `allow-file` verb.
A file-scope marker may appear anywhere in the source — there is no
"must be in first N lines" rule.

### `bca: allow` — function-scoped, all metrics (Rust)
...
### `bca: allow(metric, ...)` — function-scoped, listed metrics (Python)
...
### `bca: allow-file` — file-scoped, all metrics (JavaScript)
...
### `bca: allow-file(metric, ...)` — file-scoped, listed metrics (C++)

```cpp
/* bca: allow-file(halstead) */
...
| ------------------------- | ----------------- | ------------------------ |
| `#lizard forgives`        | Enclosing function | `bca: allow`             |
| `#lizard forgive global`  | File              | `bca: allow-file`        |
...
`bca: allow(metric, ...)` list has no Lizard analogue — every
...
| --------------------------------------- | --------------------------------- | ------------------------ |
| Silence every metric for one function   | `// bca: allow`                   | `// #lizard forgives`    |
| Silence one metric for one function     | `// bca: allow(cyclomatic)`       | (no equivalent)          |
| Silence every metric for the whole file | `// bca: allow-file`              | `// #lizard forgive global` |
| Silence one metric for the whole file   | `// bca: allow-file(halstead)`    | (no equivalent)          |

## Metric identifiers

The identifiers accepted inside `bca: allow(...)` and
`bca: allow-file(...)` are:
...
- `exit` is the suppression spelling for the threshold name `nexits`
  (the JSON field is also `nexits`) — `bca: allow(exit)` silences a
...
Unknown identifiers in a `bca: allow(...)` list emit a stderr warning
of the form
...
metrics. Unknown verbs (anything other than `allow` / `allow-file`)
...
- C-family line comments: `// bca: allow`
- C-family block comments: `/* bca: allow */`
- Rust inner doc comments: `//! bca: allow` and `/*! bca: allow */`
- Python / shell / Ruby / Perl `#` comments: `# bca: allow`
- Lisp / Lua / SQL line comments: `;; bca: allow`, `-- bca: allow`
...
`bca: allow-file` or repeat the marker on each method.
...
`kind: all` corresponds to a bare marker (`bca: allow`,
`bca: allow-file`, or any Lizard-style marker). `kind: some` carries
the explicit metric list from `bca: allow(...)` /
`bca: allow-file(...)`. Both shapes are stable serialization output
...
1. Existing `#lizard forgives` and `#lizard forgive global` markers
   ...
   the box.
2. Rewrite to the native form opportunistically. `bca: allow(...)`
   gives per-metric scoping (the Lizard form silences everything),
   reads naturally next to Rust's `#[allow(...)]` attributes, and is
   the form future audit-trail features will extend.
...
- `bca: allow(metric, reason = "...")` — audit-trail prose alongside
  the metric list, mirroring Rust's `reason = "…"` attribute argument.
- `bca: allow-next` — silence the immediately following declaration
...
discarded with a stderr warning, and `bca: allow-next` is rejected
