...
| `--vcs` | *(off)* | Append a "Change-history risk" section ranking files by VCS risk (default windows), mirroring `bca metrics --vcs`. Ignored with a warning outside a git working tree. See [`bca vcs`](vcs.md). |
