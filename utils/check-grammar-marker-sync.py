...
def read_marker(
    ...
    operator. The baseline records *which upstream version the
    vendored sources were generated from*, so `=0.23.4` and
    `0.23.4` name the same thing; comparing the raw requirement
    strings reported drift when #1151 tightened the pins without
    touching a single generated byte.
    ...
    return found.removeprefix("=") if isinstance(found, str) else found
