...
def read_marker(
    ...
    The returned version is stripped of a leading `=` requirement
    operator *and any whitespace after it*. The baseline records
    *which upstream version the vendored sources were generated
    from*, so `=0.23.4`, `= 0.23.4` and `0.23.4` all name the same
    thing; comparing the raw requirement strings reported drift when
    #1151 tightened the pins without touching a single generated
    byte. `= 0.23.4` is the spelling the two gates previously
    disagreed on — `check-excluded-manifests.py` accepted it as a
    pin while a bare `removeprefix("=")` left a leading space here
    and reported false drift.

    A compound requirement (`=0.23.4, <0.24`) names no single
    version and would still be reported as drift. That is
    deliberate and now consistent: `check-excluded-manifests.py`
    rejects it as a pin, so it cannot reach a vendored manifest in
    the first place.
    ...
    found = _scan_for_marker(data, marker)
    return found.removeprefix("=").strip() if isinstance(found, str) else found
