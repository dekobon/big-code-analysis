...
class GrammarMarkerSyncTest(unittest.TestCase):
    ...
    def test_equals_pin_matches_a_bare_baseline_version(self) -> None:
        # The baseline records which upstream version the vendored
        # sources were generated from, so `=0.25.0` and `0.25.0` name
        # the same thing. #1151 tightened these pins without touching a
        # generated byte and the literal comparison reported drift.
        script = _make_fixture(
            self.tmpdir, mozjs_version="=0.25.0", baseline=_BASELINE_MATCHING
        )
        result = _run(script)
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("OK", result.stdout)

    def test_spaced_equals_pin_matches_a_bare_baseline_version(self) -> None:
        # `= 0.25.0` is a valid cargo pin and check-excluded-manifests.py
        # accepts it as one. A bare `removeprefix("=")` left the leading
        # space in place, so this gate reported drift on a spelling its
        # sibling called compliant — the two gates disagreed by one
        # space.
        script = _make_fixture(
            self.tmpdir, mozjs_version="= 0.25.0", baseline=_BASELINE_MATCHING
        )
    ...
    def test_equals_pin_still_trips_on_a_real_version_bump(self) -> None:
        # The `=` strip must not swallow drift: an `=`-pinned marker at
        # a *different* version is still a bump without a regen. The
        # assertion is on the *stripped* form as the message renders it
        # (`repr`, hence the quotes) — asserting the bare substring
        # `0.26.0` passed with the strip removed too, since it is a
        # substring of `=0.26.0`.
        script = _make_fixture(
            self.tmpdir, mozjs_version="= 0.26.0", baseline=_BASELINE_MATCHING
        )
        result = _run(script)
        self.assertEqual(result.returncode, 1)
        self.assertIn("'0.26.0'", result.stderr)
