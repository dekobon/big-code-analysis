...
#
# CI consumes this via `bca check --config bca-thresholds.toml`. Pair with
# `.bca-baseline.toml` so existing offenders are absorbed; only regressions
# and new offenders fail the gate. See the Baselines recipe in the book
# for the bootstrap/refresh/retire flow.
#
# Editing rules:
...
/// loader requires.
const INIT_EMPTY_BASELINE_TEMPLATE: &str = "\
...
/// Scaffold the canonical adoption files in the target directory.
/// See `InitArgs` for the per-flag rationale; this function is the
/// thin orchestration layer that gates overwrites, writes the static
/// files, then optionally walks the source to produce a populated
/// `.bca-baseline.toml` (via the same `--write-baseline` code path
/// `bca check` uses).
fn run_command_init(globals: GlobalOpts, args: InitArgs, preproc: Option<Arc<PreprocResults>>) {
    ...
    let thresholds_path = target.join("bca-thresholds.toml");
    ...
    write_atomic(&thresholds_path, INIT_THRESHOLDS_TEMPLATE.as_bytes())
        .unwrap_or_else(|e| die_io("write", &thresholds_path, e));
    eprintln!("bca init: wrote {}", thresholds_path.display());
    ...
    if args.no_baseline {
        ...
        eprintln!(
            "bca init: wrote empty {} (populate via `bca check --config {} --write-baseline {}`)",
        ...
        // loaded from disk to keep this consistent with what the
        // user will use day-to-day.
        let check_args = CheckArgs {
            thresholds: Vec::new(),
            config: Some(thresholds_path.clone()),
            no_fail: false,
            no_suppress: false,
            output_format: None,
            output: None,
    ...
    eprintln!(
        "bca init: done. Next steps:\n  \
         ...
         3. Run `bca check --config {} --baseline {}` to verify the gate.",
        thresholds_path.display(),
