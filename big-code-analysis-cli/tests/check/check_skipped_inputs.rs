...
/// Overlapping seeds spell the same path differently — `-p .` walks
/// `./src/violator.rs` where `-p src` walks `src/violator.rs` — so the
/// measurement compares and dedupes in the `./`-stripped space. Without
/// that, one ignored file is counted (and listed) once per seed.
#[test]
fn overlapping_seeds_count_an_ignored_file_once() {
    let (dir, proj) = fixture(&[(".gitignore", "violator.rs\n")]);
    let src = proj.join("src");
    fs::create_dir(&src).unwrap();
    fs::write(src.join("violator.rs"), branchy("offender")).unwrap();
    fs::write(src.join("ok.rs"), CLEAN).unwrap();

    // Anchored at the project so both seeds are relative and differ
    // only in the leading `./` the walker prepends for the `.` seed.
    cli(&proj)
        .args([
            "check",
            "--no-config",
        ...
        .success()
        .stderr(predicate::str::contains("1 file not checked (1 ignored)"))
        .stderr(predicate::str::contains("2 files not checked").not());
    drop(dir);
}

/// A directory an ignore rule prunes under one seed but that another
/// seed names outright *was* walked, and its files reached the gate —
/// reporting it as "not walked" would contradict the violation the same
/// run just printed.
#[test]
fn a_directory_named_as_its_own_seed_is_not_reported_as_pruned() {
    let (dir, proj) = fixture(&[("ok.rs", CLEAN), (".gitignore", "build/\n")]);
    let build = proj.join("build");
    fs::create_dir(&build).unwrap();
    fs::write(build.join("gen.rs"), branchy("offender")).unwrap();
    ...
    cli(&proj)
        .args([
            ...
            "--threshold",
            "cyclomatic=1",
            // Dir clauses only surface when listing, so the absence
            // assertion below can only mean "not classified as pruned".
            "--report-skipped",
        ])
        .assert()
        // The gate analysed `build/gen.rs`, so it must not also claim
        // the directory went unwalked.
        .code(2)
        .stdout(predicate::str::contains("offender"))
        .stderr(predicate::str::contains("not walked").not());
    drop(dir);
}

/// Pruned directories are absent from the default summary: every real
/// checkout has an ignored build tree, so a default that counted them
/// would print on every run and bury the file-count signal. The
/// `--report-skipped` sibling test above pins that the information is
/// still available on request.
#[test]
fn ignored_directories_stay_out_of_the_default_summary() {
    let (dir, proj) = fixture(&[("ok.rs", CLEAN), (".gitignore", "build/\n")]);
    let build = proj.join("build");
    fs::create_dir(&build).unwrap();
    fs::write(build.join("junk.o"), b"\x7fELF").unwrap();
...
/// A generated marker in a file no parser owns is not a gate bypass —
/// the file would have been dropped at language resolution anyway.
/// `Cargo.lock` opens with `@generated`, so without this rule the
/// summary would fire on every Rust repository, permanently.
#[test]
fn generated_marker_in_unparsed_file_is_not_counted() {
    let (dir, proj) = fixture(&[
        ("ok.rs", CLEAN),
        (
            "Cargo.lock",
...
/// An ignored file the caller names explicitly — in absolute form, the
/// shape the per-edit agent hooks use — bypasses ignore rules and is
/// analyzed (#726), so the summary must not also claim it went
/// unchecked while its violation prints in the same run.
#[test]
fn explicitly_named_ignored_file_is_not_reported_as_skipped() {
    let (dir, proj) = fixture(&[
        ("violator.rs", &branchy("offender")),
        ("ok.rs", CLEAN),
        (".gitignore", "violator.rs\n"),
    ]);
    let absolute = proj.join("violator.rs");

    check(&dir, &proj, &["--paths", absolute.to_str().unwrap()])
        .code(2)
        .stdout(predicate::str::contains("offender"))
        .stderr(predicate::str::contains("not checked").not());
}
