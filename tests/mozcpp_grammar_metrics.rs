...
/// `macro_annotation` overlay must not derail class / method recognition,
/// so the `MozcppCode` `Checker::is_func_space` (`ClassSpecifier`),
/// `Getter::get_space_kind` (Class), and `Npm` arms all fire.
#[test]
...
/// mozcpp overlay parses; upstream `tree-sitter-cpp` ERROR-cascades on it.
/// Ensure the enclosing function's metrics are computed rather than lost
/// to an error cascade.
#[test]
...
/// agree — the overlay only adds rules, it does not change how ordinary
/// C++ is measured. (Complements `tests/cpp_mozcpp_parity.rs`, here via a
/// class so the `npm`/`wmc` class arms are part of the comparison.)
#[test]
fn mozcpp_matches_cpp_on_plain_class() {
    ...
    // (`width`; the private `cached` is not an NPA), so the parity above
    // compares real non-zero values.
...
/// ABC condition counting must also match across the two grammars. This
/// guards the `cpp_inspect_container` / `cpp_count_unary_conditions`
/// helpers, which (unlike the name-based `npa` helpers) still key off the
/// `Cpp` enum discriminants; they happen to agree with Mozcpp today only
/// because the relevant `kind_id`s coincide. A future grammar bump that
/// shifts those ids would silently break Mozcpp ABC — this test fails if
/// that happens. (Latent fragility tracked in #732.)
#[test]
