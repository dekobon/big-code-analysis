...
// names the grammar it needs. The glob list (`*.php`) is what
// decides it, and a build without that grammar scores every file
// zero rather than matching the snapshot (#1472).
// test-lang-gates: hand-written(php) — the corpus walk picks a language
//     per file from its extension, so the glob list decides it and
