...
//!
//! `big_code_analysis::FromPathError`, the richer error of the
//! file-backed `Ast::from_path`, lives in the root crate's
//! `from_path_error` module; this one is the parse-layer error every
...
/// current build (see #252). The example demonstrates the
/// exhaustive-with-`_` match shape that callers should adopt to stay
/// forward-compatible with future variants.
///
/// [`AnyParser::parse`](crate::langs::AnyParser::parse) stands in for
/// `big_code_analysis::analyze` here — it is the same dispatch, one
/// layer down, and it is reachable from this crate alone. The match
/// arms are the ones an `analyze` caller writes.
///
/// ```
/// use big_code_analysis_ast::{AnyParser, MetricsError, LANG};
///
/// let result = AnyParser::parse(LANG::Cpp, b"int a = 42;".to_vec(), None, None);
///
/// match result {
///     Ok(_parser) => {
///         // The `cpp` feature is enabled in this build.
///     }
...
pub enum MetricsError {
    /// The walker produced no top-level `big_code_analysis::FuncSpace`.
    ///
    ...
    /// (`big_code_analysis::analyze`, `big_code_analysis::Ast::parse`,
    /// `big_code_analysis::Ast::from_tree_sitter`, and
