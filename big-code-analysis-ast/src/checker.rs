...
//! The [`Checker`] classification predicates and their per-language impls.
...
///
/// `#[doc(hidden)]` for the reason the crate root gives: this is
/// plumbing, and the surface the README points a reader at is `LANG` /
/// `Node` / `SpaceKind`, not a 200-method classifier trait.
#[doc(hidden)]
pub trait Checker {
    /// Whether `node` is a comment.
    #[inline]
    #[must_use]
    ...
    /// Whether `node` opens a space: a function, closure, or one of the
    /// containers (class, struct, trait, impl, namespace, interface).
    #[inline]
    #[must_use]
    ...
    /// [`is_func`](Self::is_func) on the grammars that express both. Takes `ancestors`
    ...
    /// Whether `node`, met among a parameter list's children, is
    /// punctuation or a keyword rather than a parameter (`nargs` skips it).
    #[inline]
    #[must_use]
    ...
    /// negative filters in `big_code_analysis::nargs` counted it as one — `f` and
    ...
    /// Whether `node` is a primitive type name (`int`, `double`, …),
    /// which Halstead keys by text so each distinct primitive counts once.
    #[inline]
    #[must_use]
    ...
    /// Whether `node` is, or contains, a tree-sitter `ERROR` node.
    #[must_use]
    ...
    /// Source-aware variant of [`is_func`](Self::is_func). Same rationale as
    /// [`is_func_space_with_code`](Self::is_func_space_with_code) (#275).
    #[inline]
    #[must_use]
...
/// reaches this path.
#[must_use]
pub fn java_anonymous_class_body<'a>(node: &Node<'a>) -> Option<Node<'a>> {
...
#[must_use]
pub fn csharp_accessor_count(node: &Node) -> usize {
    node.children()
...
/// arities.
#[must_use]
pub fn c_family_void_parameter(param: &Node, code: &[u8]) -> bool {
