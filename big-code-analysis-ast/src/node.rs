...
//! Tree-sitter wrappers: `Tree`, [`Node`], [`Ancestors`], and the cursors.
...
crate::observation::counter!(
    node_resolved_sibling_lookups,
...
// module's own tests. The accessor is `pub` (not `pub(super)`)
// precisely so the last one can be asserted from where it lives — see
// `crate::observation`.
crate::observation::counter!(child_scan_cursors, cfg(any(test, feature = "test-support")));

/// A parsed source tree wrapping a [`tree_sitter::Tree`].
///
/// The "open parse seam" (see issue #251) is reached by external
/// callers through `big_code_analysis::Ast::from_tree_sitter`, which accepts a
...
impl Tree {
    /// # Panics
    ///
    /// When `T`'s per-language Cargo feature is disabled in this build.
    ///
    /// [`AnyParser::parse`](crate::langs::AnyParser::parse) and
    /// [`AnyParser::from_tree`](crate::langs::AnyParser::from_tree)
    /// cfg-gate each `LANG::*` arm and return
    /// `Err(MetricsError::LanguageDisabled)` before reaching here, so
    /// every path through `big_code_analysis::Ast` is safe. Since
    /// #1376, though, `Parser<T>` and [`ParserTrait::new`] are `pub` in
    /// this crate, so a caller that names a `*Parser` alias directly
    /// bypasses that gate — check
    /// [`LANG::is_enabled`](crate::LANG::is_enabled) first, or go
    /// through `AnyParser` and match on the error.
    pub(crate) fn new<T: LanguageInfo>(code: &[u8]) -> Self {
        let language = T::lang().get_ts_language().expect(
            "invariant: the caller checked LANG::is_enabled, or reached this through AnyParser",
        );
...
impl<'a> Node<'a> {
    ...
    /// lifetime. Suitable as a map key for per-node walk state.
    #[inline]
    #[must_use]
    ...
    /// The grammar's name for this node's kind (`"function_item"`,
    /// `"identifier"`, …). Several distinct [`kind_id`](Self::kind_id)s
    /// can share one name — see the aliasing note on `kind_id`.
    #[inline]
    #[must_use]
    ...
    /// The numeric kind of this node in its grammar's symbol table.
    ///
    /// This is what the per-language token enums in [`languages`]
    /// (`Rust::FunctionItem as u16`, …) and every classifier match on.
    /// The value is an index into the pinned grammar and moves whenever
    /// that grammar is bumped; it is not stable across versions.
    ///
    ...
    /// The 0-based byte offset one past this node's last byte.
    #[inline]
    #[must_use]
    ...
    /// The 0-based `(row, column)` where this node starts.
    #[inline]
    #[must_use]
    ...
    /// The 0-based `(row, column)` one past this node's end.
    #[inline]
    #[must_use]
    ...
    /// The 0-based row this node starts on.
    #[inline]
    #[must_use]
    ...
    /// The 0-based row of this node's end position. A node ending at
    /// column 0 does not occupy that row; see [`end_line`](Self::end_line).
    #[inline]
    #[must_use]
    ...
    #[inline]
    #[must_use]
    pub fn has_sibling(&self, ancestors: Ancestors<'a, '_>, id: u16) -> bool {
        ancestors
            .parent(self)
    ...
    #[inline]
    #[must_use]
    pub fn wraps_any(&self, ids: &[u16]) -> bool {
        self.children().any(|c| ids.contains(&c.kind_id()))
    }

    /// The number of direct children, named and anonymous alike.
    #[inline]
    #[must_use]
    ...
    /// anonymous token such as a punctuation or keyword literal. Walks
    /// use it to skip tokens like the leading `|` in an or-pattern.
    #[inline]
    #[must_use]
    ...
    /// The direct child at index `pos`, counting named and anonymous
    /// children alike. Prefer [`child_by_field_name`](Self::child_by_field_name)
    /// for any slot the grammar can precede with an optional preamble.
    #[inline]
    #[must_use]
    ...
    /// [`children`]: Self::children
    #[inline]
    pub fn children_with<'c>(&self, cursor: &'c mut Cursor<'a>) -> ChildrenWith<'c, 'a> {
    ...
    pub fn count_specific_ancestors<C: Checker>(
        &self,
    ...
    /// nesting depth (#1096).
    pub fn parent_grandparent_match(
        &self,
        ancestors: Ancestors<'a, '_>,
    ...
    /// same unaltered vocabulary `big_code_analysis::Ast::root_node` exposes, not
    /// the `Alterator`-curated kinds `big_code_analysis::Ast::dump` emits. This is
...
#[derive(Clone, Copy, Debug)]
pub struct Ancestors<'tree, 'chain>(Option<&'chain [Node<'tree>]>);
...
impl<'tree, 'chain> Ancestors<'tree, 'chain> {
    ...
    #[inline]
    #[must_use]
    pub const fn known(chain: &'chain [Node<'tree>]) -> Self {
        Self(Some(chain))
    }
    ...
    #[must_use]
    pub fn checked(chain: &'chain [Node<'tree>], node: &Node<'tree>) -> Self {
    ...
    #[inline]
    #[must_use]
    pub fn parent_has_kind(self, node: &Node<'tree>, kind: u16) -> bool {
    ...
    #[inline]
    #[must_use]
    pub fn previous_sibling(self, node: &Node<'tree>) -> Option<Node<'tree>> {
    ...
    #[inline]
    #[must_use]
    pub fn iter(self, node: &Node<'tree>) -> AncestorIter<'tree, 'chain> {
        match self.0 {
...
pub enum AncestorIter<'tree, 'chain> {
...
impl<'a> Cursor<'a> {
    /// Re-seats the cursor on `node`.
    #[inline]
    pub fn reset(&mut self, node: &Node<'a>) {
        self.0.reset(node.0);
    }

    /// Moves to the next sibling; `false` when there is none.
    #[inline]
    pub fn goto_next_sibling(&mut self) -> bool {
        self.0.goto_next_sibling()
    }

    /// Moves to the first child; `false` when there is none.
    #[inline]
...
mod tests {
    ...
    /// be a test failure rather than a silent allocation per node. The
    /// metric and suppression walks are guarded the same way from
    /// `big-code-analysis`, where they live.
    ///
