...
fn parsing_a_cpp_file_never_owns_the_macro_set() {
    std::thread::spawn(|| {
        ...
        let parsed = crate::langs::AnyParser::parse(
            crate::LANG::Cpp,
            b"int f(int x) { return DBG ? FOO : x; }".to_vec(),
            Some(&path),
            Some(pr),
        )
        .expect("cpp feature enabled");
        // The masking pass rewrites each macro name to a `$` run of the
        // same length, so the parsed bytes carry two `$$$` runs and
        // neither name. Deleting the C-family arm of `get_fake_code`
        // leaves the names in place and fails here.
        let code = parsed.code();
        assert!(
            !code.windows(3).any(|w| w == b"DBG" || w == b"FOO"),
            "macro names must be masked before parsing: {}",
            String::from_utf8_lossy(code)
        );
        assert_eq!(
            code.windows(3).filter(|w| *w == b"$$$").count(),
            2,
            "each three-byte macro name masks to one `$$$` run: {}",
            String::from_utf8_lossy(code)
        );
